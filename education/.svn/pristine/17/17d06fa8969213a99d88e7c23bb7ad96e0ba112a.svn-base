...
function checkData() {
	getNames()
	var url = pathHeader + "/sys/checkUploadSchool.do";
	$.ajax( {
		url : url,
		data : {
			path : $("#savePath").val()
...
function download() {
	...
	$("#downIframe", document.body).attr("src",pathHeader + "/sys/downSchoolTemplate.do");
}
 ...
 */
function submitData() {
	var url = pathHeader + "/sys/submitUploadSchool.do";
	$.ajax( {
		url : url,
		data : {
			path : $("#toDBPath").val()
