...
function checkData(){
	getNames()
	var url = pathHeader + "/edupm/projectmanage/batchSignUpStep1.do";
	$.ajax({
		url:url,
		data:{path:$("#savePath").val(),projectId:proId},
		async:false,
 ...
 */
function submitData(){
	var url = pathHeader + "/edupm/projectmanage/batchSignUpStep4.do";
	$.ajax({
		url:url,
		data:{path:$("#toDBPath").val(),projectId:proId,type:type},
		type:"post",
		async:true,
		success:function(data){
			if(data.success){
