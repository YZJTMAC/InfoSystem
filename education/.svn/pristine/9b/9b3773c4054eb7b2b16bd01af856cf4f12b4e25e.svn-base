...
function eduUser(data) {
	...
	if (obj.realName == null || obj.realName.length == 0
		...
		$('#teacherName').val(obj.realName + "老师的资料信息");
	}
	...
	if (obj.schoolName == null || obj.schoolName.length == 0
		...
		$('#selectSchoolId').val(obj.schoolId);
		provinceId = obj.userProvinceId;
		provinceName = obj.userProvinceName;
		cityId = obj.userCityId;
		cityName = obj.userCityName;
		districtId = obj.userDistrictId;
		districtName = obj.userDistrictName;
	 ...
	 var infos ="" ;
			if(auditInfo != null){
				...
				if(auditInfo.status==6){
					...
					auditStatus = "审核未通过";
				}else if(auditInfo.status >= 8 && auditInfo.status != 44) {
					auditStatus = "审核通过";
				}
				var auditAuditor = "";
