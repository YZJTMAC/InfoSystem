...
function getTdEHtml(obj ,index) {
	...
	function optionFormatter(rows){
		...
		var remove = "<A class='tablelink  ' href='###' onclick='deleteNotrainInfo("+rows.id+")' >删除 </A>";
		var returnStr = "";
		returnStr += edit;
		returnStr += remove;
		if(rows.status == 1) {
...
function edit(id) {
	var openUrl= pathHeader + "/furtheredu/certificate/notrainUpdPage.htm?click=6&id="+id;
	showWindow(openUrl);
}
