...
function projectList(flag){
	...
	if(flag == 1){
		...
		projectScopeName="自由报名项目";
	}
	var url = pathHeader+ "/teacher/teacherProject/queryAllProject.do";
	$('#proList').html("");
	$.ajax({
		url:url,
		type:"post",
...
function getTdHtml(obj){
	...
	gridHtml += "<td>"+optionSignUpWay(obj)+"</td>";
	gridHtml += "<td>"+planNum+"</td>";
	gridHtml += "<td>"+optionFormatter(obj)+"</td></tr>";
	
	return gridHtml;
	function optionSignUpWay(row){
		var returnValue = "";
		if(row.signUpWay == '1'){
			returnValue = "组织报名";
		}else if(row.signUpWay == '2'){
			returnValue = "自由报名";
		}else if(row.signUpWay == '3'){
			returnValue = "层级报名";
		}
		return returnValue;
	}
...
//				}
				...
				if(row.teacherOkOrNo=="Ok"&row.status==13){
