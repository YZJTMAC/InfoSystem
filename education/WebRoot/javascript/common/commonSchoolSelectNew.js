...
function validateRole() {
	if(lastSelect != null && lastSelect.length > 0) {
		...
		if(areaA == "provinceId"||areaA == "cityId"||areaA == "districtId") {
			$(".zhuyi").show();
			return false;
		}
	}
...
function areaList(belong, area, text){
	$(".zhuyi").hide();
	var url = pathHeader + "/sys/curLoginAreaListNew.do";
	$.ajax({
		url:url,
		data:{
			area:area,
	...
	function dHtml(obj) {
		...
		for(var i=0; i<obj.length; i++){
			...
			if(obj[i].schoolCount > 0) {
				htmlStr += "<a href='###' onclick='querySchool("+obj[i].code+");districtClick("+obj[i].code+", \""+obj[i].name+"\" );' class='tablelink'>" + obj[i].name ;
				htmlStr += "<img src='"+pathHeader+"/images/more.jpg' width='13' height='13'></a>";
			} else {
				htmlStr += "<a href='###' onclick='districtClick("+obj[i].code+", \""+obj[i].name+"\" );' class='tablelink'>" + obj[i].name + "</a>";
			}
			htmlStr += "</dd>";
		}
		return htmlStr;
...
/**
	 ...
	 * <!-- 教师调动页面重新单独使用一个查询地区学校的页面，因为教师调动需要跨市区校 -->
	 */
function querySchool(districtId) {
	$(".zhuyi").hide();
	var schoolTypeId = $('input:radio[name="schoolTypeId"]:checked').val();
	$.ajax({
		url: pathHeader + "/sys/querySchoolNew.do",
		data:{
				...
				if(obj.length > 0){
					...
					var htmlStr = "";
						for(var i=0; i<obj.length; i++){
							...
							if(i == obj.length-1 && x != 3) {
								for(var y = 0; y < (3 - (obj.length%3)); y++) {
									htmlStr += "<td width=\"200\">&nbsp;</td>";
								}
								x =3;
							}
							if(x == 3) {
