 ...
 */
function download() {
	$("#downIframe", document.body).attr("src",pathHeader + "/master/downMasterTemplate.do");
}
...
function checkData() {
	getNames()
	var url = pathHeader + "/master/checkUploadMaster.do";
	$.ajax( {
		url : url,
		data : {
			path : $("#savePath").val()
 ...
 */
function submitData() {
	var url = pathHeader + "/master/submitUploadMaster.do";
	$.ajax( {
		url : url,
		data : {
			path : $("#toDBPath").val()
